...
TEST(BookTest, BasicBookCreation) {
    ...
    EXPECT_EQ(book->getStockQuantity(), 50);
    EXPECT_DOUBLE_EQ(book->getPrice(), 19.99);
    EXPECT_TRUE(book->isInStock());
}

TEST(BookTest, StockOperations) {
    auto publisher = std::make_shared<Publisher>("Test Pub", "test@pub.com", 2000);
    auto book = std::make_shared<Book>(
        ISBN("9783161484100"),
        BookTitle("Test Book", "", "EN"),
    ...
    book->updateStock(5);
    EXPECT_EQ(book->getStockQuantity(), 15);
    
    book->updateStock(-3);
    EXPECT_EQ(book->getStockQuantity(), 12);
    
    EXPECT_THROW(book->updateStock(-20), InsufficientStockException);
}
...
TEST(BookTest, BookEquality) {
    ...
    auto book2 = std::make_shared<Book>(
        ISBN("9783161484100"), // тот же ISBN
        BookTitle("Book 2", "", "EN"), // другое название
        BookMetadata(2023, "RU", 2, ""), // другие метаданные
    ...
    auto book3 = std::make_shared<Book>(
        ISBN("0306406152"), // другой ISBN (валидный ISBN-10)
        BookTitle("Book 1", "", "EN"),
        BookMetadata(2024, "EN", 1, ""),
        PhysicalProperties(300, 200, 130, 20, 250, 
                         PhysicalProperties::CoverType::PAPERBACK, "Paper"),
    ...
    EXPECT_EQ(*book1, *book2); // тот же ISBN
    EXPECT_NE(*book1, *book3); // разные ISBN
}
...
TEST(BookCollectionTest, BasicOperations) {
    ...
    auto book1 = std::make_shared<Book>(
        ISBN("0306406152"),  // ВАЛИДНЫЙ ISBN-10
        BookTitle("Book 1", "", "EN"),
        BookMetadata(2024, "EN", 1, ""),
        PhysicalProperties(300, 200, 130, 20, 250, 
                         PhysicalProperties::CoverType::PAPERBACK, "Paper"),
    ...
    auto book2 = std::make_shared<Book>(
        ISBN("0451524934"),  // ВАЛИДНЫЙ ISBN-10
        BookTitle("Book 2", "", "EN"),
        BookMetadata(2024, "EN", 1, ""),
        PhysicalProperties(300, 200, 130, 20, 250, 
                         PhysicalProperties::CoverType::PAPERBACK, "Paper"),
...
TEST(IntegrationTest, CompleteFlow) {
    ...
    EXPECT_EQ(book->getStockQuantity(), 120);
    EXPECT_DOUBLE_EQ(book->getAverageRating(), 4.5);
    EXPECT_TRUE(book->isInStock());
