...
Book::Book(const ISBN& isbn, const BookTitle& title, const BookMetadata& metadata,
    ...
    : isbn(isbn), title(title), metadata(metadata), physicalProps(physicalProps),
      ...
      price(price), statistics(0, 0, 0.0, 0, "1400-01-01") {
    if (price < 0) {
...
Book::Book(const ISBN& isbn, const BookTitle& title): isbn(isbn), title(title), price(0), metadata(1400, "NO"), 
           physicalProps(1,1,1,1,1,PhysicalProperties::CoverType::HARDCOVER,"NO"), genre(Genre::Type::OTHER),
           publisher(std::make_shared<Publisher>("No", "example@mail.info", 1400)), condition(BookCondition::Condition::NEW), 
           statistics(0, 0, 0.0, 0, "1400-01-01"){}

ISBN Book::getISBN() const noexcept {
    return isbn;
}

std::string Book::getISBNstring() const noexcept{
    return isbn.getFormattedCode();
}
